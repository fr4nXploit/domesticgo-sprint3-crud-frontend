...
@Component({
  selector: "app-servicio-eliminar",
  standalone: true,
    ...
    <div class="dialog-container">
      ...
      <div class="dialog-content">
        <p class="dialog-message">
          ...
          <strong>#{{data.idServicio}} - {{data.tipoServicio}}</strong>?
        </p>
...
export class ServicioEliminarComponent {
  eliminando = false

  constructor(
    public dialogRef: MatDialogRef<ServicioEliminarComponent>,
  ...
  eliminar(): void {
    ...
    this.servicioService.eliminar(this.data.idServicio).subscribe({
      next: () => {
      ...
      error: (error) => {
        this.snackBar.open("Error al eliminar servicio", "Cerrar", {
          duration: 3000,
        })
