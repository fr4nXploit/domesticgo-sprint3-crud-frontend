...
@Component({
  selector: "app-contrato-eliminar",
  standalone: true,
    ...
    <div class="dialog-container">
      ...
      <div class="dialog-content">
        <p class="dialog-message">
          ...
          <strong>#{{data.idContrato}}</strong>?
        </p>
        <p class="dialog-description">{{data.descripcionContrato | slice:0:100}}{{data.descripcionContrato.length > 100 ? '...' : ''}}</p>
...
export class ContratoEliminarComponent {
  eliminando = false

  constructor(
    public dialogRef: MatDialogRef<ContratoEliminarComponent>,
  ...
  eliminar(): void {
    ...
    this.contratoService.eliminar(this.data.idContrato).subscribe({
      next: () => {
        this.snackBar.open("Contrato eliminado correctamente", "Cerrar", {
          duration: 3000,
        })
        this.dialogRef.close(true)
      },
      error: (error) => {
        this.snackBar.open("Error al eliminar contrato", "Cerrar", {
          duration: 3000,
        })
